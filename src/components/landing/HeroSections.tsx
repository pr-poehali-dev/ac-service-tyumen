      ...
      <section id="hero" ref={heroSection.ref} className="relative min-h-screen flex items-center pt-20 sm:pt-16 overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 py-12 sm:py-20 lg:py-24">
          <div className="max-w-2xl">
            ...
            <h1 className={`font-oswald text-4xl sm:text-5xl lg:text-7xl font-black leading-[1.05] mb-5 sm:mb-6 ${heroSection.inView ? 'animate-fade-in-up delay-100' : 'opacity-0'}`}>
              ПРОФЕССИОНАЛЬНЫЙ<br />
              <span className="text-neon-blue text-glow-blue">ТЕХНИЧЕСКИЙ</span><br />
              СЕРВИС
            </h1>

            <p className={`text-base sm:text-lg text-foreground/70 leading-relaxed mb-8 sm:mb-10 max-w-lg ${heroSection.inView ? 'animate-fade-in-up delay-200' : 'opacity-0'}`}>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-5">
            {BLOG_POSTS.map((post, i) => (
              <article key={i} className={`card-service rounded-2xl p-5 sm:p-7 cursor-pointer ${blogSection.inView ? 'animate-fade-in-up' : 'opacity-0'}`}
