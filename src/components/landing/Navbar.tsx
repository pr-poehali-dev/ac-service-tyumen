    ...
    <header className="fixed top-0 left-0 right-0 z-50 glass border-b border-border">
      <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <a href="tel:+74951234567" className="hidden sm:flex items-center gap-2 text-sm text-foreground/70 hover:text-neon-blue transition-colors">
          ...
          <button onClick={() => setMobileOpen(!mobileOpen)} className="lg:hidden p-2 text-foreground/75">
        ...
        <div className="lg:hidden glass border-t border-border px-6 py-4 space-y-3">
          {NAV_ITEMS.map(n => (
            <a key={n.href} href={n.href} onClick={() => setMobileOpen(false)}
              className="block py-2 text-foreground/75 hover:text-foreground transition-colors font-medium">
              {n.label}
            </a>
          ))}
